...
@socketio.on("join", namespace="/trasup")
def join(message):
    try:
...
@socketio.on("my_room_event", namespace="/trasup")
...
@socketio.on("disconnect_request", namespace="/trasup")
...
@socketio.on_error("/trasup")
def error_handler(e):
    pass

@app.route("/" + trasup.home_path)
def home():
    return render_template("home.html", async_mode=socketio.async_mode)

if __name__ == 'core.sockets':
    socketio.run(app, host= '0.0.0.0', port=trasup.app_port, debug=False)
